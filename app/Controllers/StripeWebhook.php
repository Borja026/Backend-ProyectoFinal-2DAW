...
class StripeWebhook extends ResourceController
{
    ...
    //     $endpoint_secret = $_ENV['STRIPE_WEBHOOK_SECRET']; // clave secreta de tu webhook (Stripe Dashboard)

    //     try {
    //         $event = \Stripe\Webhook::constructEvent($payload, $sig_header, $endpoint_secret);
    //     } catch (\UnexpectedValueException $e) {
    //         // Error en la carga del cuerpo
    ...
    //     if ($event->type === 'checkout.session.completed') {
    //         $session = $event->data->object;
    //         $reserva = json_decode($session->metadata->reserva ?? '{}', true);

    //         if (!$reserva || !isset($reserva['correoClientes'])) {
    //             return $this->fail('Datos de reserva no válidos');
    //         }

    //         $model = new \App\Models\PistasClientesModel();
    ...
    //         if ($totalFinal > 4) {
    //             return $this->respond(['message' => 'No se pudo guardar. Excede el límite de 4 personas.']);
    //         }
