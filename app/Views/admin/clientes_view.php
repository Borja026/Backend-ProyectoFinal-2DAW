...
<?= view('admin/menu') ?>
  ...
  <form action="<?= base_url('admin/clientes/guardar') ?>" method="post">
    ...
    <label>Correo:</label>
    <input type="email" name="correo" value="<?= isset($clienteEditando) ? $clienteEditando['correo'] : '' ?>"
    ...
    <input type="text" name="telefono" value="<?= isset($clienteEditando) ? $clienteEditando['telefono'] : '' ?>"
      required>
        ...
        <tr>
          ...
          <td class="acciones">
            ...
            <a href="<?= base_url('admin/clientes/eliminar?correo=' . urlencode($cliente['correo'])) ?>"
              onclick="return confirm('¿Seguro que quieres eliminar este cliente?')">Eliminar</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
